...
University of Oxford, Oxford, OX1 3QX, UK
	...
	\\ \begin{tabular}{m{8cm}l}\\\textbf{Number of Figures:} 8
		...
		\\ \textbf{Abstract word count:} 236
		 &  \textbf{Introduction word count:} 645
		\\ \textbf{Discussion word count:} 1491
		\end{tabular}
...
\renewcommand{\baselinestretch}{1.4}\normalsize
		...
		\begin{figure}[h]
			...
			\caption{Detailed analysis of mouse retinal ganglion cell recordings. \textbf{A} Fraction of spikes in bursts, and \textbf{B} Mean burst duration as found by each burst detector. Each electrode was counted as one data point in the box plots. The legend in A applies to both A and B. \textbf{C} Bursts detected by each burst detector over a 600$\,$s sample of a P15 spike train, and \textbf{D} 20$\,$s sample showing the first bursting episode from the same spike train. Horizontal bars in C and D show the bursts detected by each method. Blue bars above the spike train represent the bursts annotated by a human observer.  }
			\label{RGC_results}
		\end{figure}
		\begin{figure}[h]
			\centering
			\fbox{\includegraphics[width=116mm]{hiPSN_analysis.pdf}}
			\caption{Analysis of recordings of networks of human induced pluripotent stem cell-derived neurons. \textbf{A} Mean burst duration, \textbf{B} Bursts per minute, \textbf{C} Fraction of spikes in bursts, and \textbf{D} Coefficient of variation of interburst intervals (CV of IBI). Each data point in the box plots is the mean value across all spike trains from one recording.}
			\label{hiPSN_results}
		\end{figure}
		\begin{figure}[h]
			\centering
