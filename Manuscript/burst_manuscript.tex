...
University of Oxford, Oxford, OX1 3QX, UK
	...
	\\Any series of consecutive spikes fulfilling five user-inputted threshold parameters are classified as bursts. For our purposes, the values of the parameters were those specified in the NeuroExplorer Manual \cite{NEmanual}, and shown in Table \ref{param_vals}.
	...
	\\ \\ Another desirable property for the burst detectors was that they did not assume that ISIs on a spike train follow a specific statistical distribution (D2). There is no consensus on which type of statistical distribution best represents underlying spike train activity%(ref?)
	...
	MEA recordings from \citetext{Demas2003}, a study which examined the changes in spontaneous retinal activity in both normal and dark-reared mice over development, were re-analysed using the burst detection methods in our study. The sensitivity and specificity of each burst detector at a range of parameter values was found using the approach outlined in the methods, and averaged across the five annotated spike trains from each recording, to create the ROC curves for each age shown in Figure \ref{ROC}. 
	...
	In cases when this distinction is not apparent, we recommend the PS method as an effective alternative with only one input parameter. Due to its tendency to overestimate burst durations in some circumstances, however, post hoc screening for outliers in terms of burst duration is advisable when using this method.  
	...
	\begin{table}[h]
		...
		\begin{tabular}{|p{5cm}|l|l|}
			...
			\\ \hline
			Cumulative Moving  & $\alpha_1 (\alpha_2)$ & 1 (0.5) if skew $<1$
			\\ 
			...
			\rowcolor{Gray} Rank Surprise& Largest allowed ISI in burst & 75th percentile of ISIs\\
			...
			\\ \hline
			\rowcolor{Gray} Hidden Semi-Markov  & Probabilistic cutoff & 0.5
			\\ \rowcolor{Gray} Model \cite{Tokdar2010}& Other parameters (N=23) & As per paper 
			\\ \hline
	...
	\begin{table}[h] \centering 	\rowcolors{1}{white}{gray!25}
		\begin{tabular}{|l|m{13cm}|}
			...
			\\D10 & Noisy train: The method should classify a high number of within-burst spikes as bursting and a low number of interburst spikes as bursting, in trains containing both bursts and noise spikes
			\\ \hline
	...
	\begin{table}[h] \centering \small{
			\begin{tabular}{|l|l|l|c|}
				...
				\\ \hline
			\end{tabular}}
			\caption{Models and parameter values used to generate simulated data for each desirable property. Each spike train was 300$\,$s duration, and the number, $N$, of simulated trains was 100, unless otherwise stated.} \label{sim_params}
		\end{table}
		...
		\begin{figure}
			...
			\caption{Examples of one minute spike trains from recordings of mouse retinal ganglion cell at each postnatal day. Horizontal blue bars represent bursts annotated by a human observer. Scale bar represents 5$\,$s of activity. Raw data available from \protect\citetext{Eglen2014a}.}
			\label{RGC_eg}
		\end{figure}
		\begin{figure}[h]
			\centering
		...
		\begin{figure}[h]
			...
			\caption{Detailed analysis of mouse retinal ganglion cell recordings. \textbf{A} Percentage of spikes in bursts, and \textbf{B} Mean burst duration found by each burst detector. \textbf{C} Bursts detected by each method over a 1000$\,$s sample of a P15 spike train, and \textbf{D} 20$\,$s sample showing the first bursting episode from the same spike train. Each electrode was counted as one data point in the box plots. The legend in \textbf{A} applies to both \textbf{A} and \textbf{B}.}
			\label{RGC_results}
		\end{figure}
		\begin{figure}[h]
			\centering
