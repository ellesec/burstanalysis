...
run.method<-function(spike.train, method, cutoff=NULL) {
  bursts <- switch(method, mi = MI.method(spike.train), 
                  ...
                  cma=CMA.method(spike.train, brs.incl=cutoff), hennig=hennig.method(spike.train), mean.isi=MISI.method(spike.train), logisi.pasq=logisi.pasq.method2(spike.train), clust=dbscan.cluster(spike.train, cutoff), clust2=cluster.method(spike.train), stop(method, " : no such method for burst analysis"))
